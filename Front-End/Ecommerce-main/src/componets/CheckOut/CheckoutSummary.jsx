...
const CheckoutSummary = ({ cartItems, subtotal, shipping, total, language }) => {
  const text = {
    EN: {
...
<h3 className="font-medium text-gray-800 text-sm truncate">
  {typeof item.name === "object" && item.name !== null ? item.name[language] : item.name}
            ...
            <div className="text-right">
              <p className="font-semibold text-habesha_blue">${(item.quantity * item.price).toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-200 pt-4 space-y-3">
        ...
        <div className="border-t border-gray-200 pt-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-2xl font-bold bg-gradient-to-r from-habesha_blue to-blue-400 bg-clip-text text-transparent">
              ${total.toFixed(2)}
            </span>
          </div>
        </div>
